...
% Input
% - convergence_array: an array ix2 containing, for each row:
...
% - knwon: true if the true optimum is not known, false otherwise
function [] = showLog(it, budget, error, fit, known)
...
% Input:
% - pop: µx(d+1) matrix of parents (last column is fitness)
% - sigma_global: A scalar representing the current global step size
% - lambda: number of offspring to generate
% - bounds: 1x2 vector [LB, UB]
% Output:
% - off: λxd matrix of generated offspring
% - parent_indices: λx1 vector storing which parent index was used for each offspring
