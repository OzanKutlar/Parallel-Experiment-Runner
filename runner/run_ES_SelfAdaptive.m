%% Run a (mu lambda) Evolution Strategy (ES) with Self Adaptation on a given single-objective optimization problem
% Input
...
function [best, error, runtime, convergence_array] = run_ES_SelfAdaptive(op, algo)
    close all;
    rng shuffle;
    ...
    % learning rates
    algo.tau = 1/sqrt(2*sqrt(op.dim));                % local learning rate for step-size adaptation 
    algo.tau_prime = 1/sqrt(2*op.dim);    % global learning rate for step-size adaptation

    % allocate mutation strengths for each parent
    sigma_pop = algo.sigma * ones(algo.mu, op.dim);

    convergence_array = zeros(round((op.maxFE/op.dim)/algo.lambda),2);    % convergence array for plotting
    ...
    if ~strcmp(algo.survival.schema,"(µ,λ)")
        ...
        [best, ~] = Shared.findBestWorst(matPool, op.isMin);                           % update best
    ...
    while op.maxFE > 0
        ...
        [off, sigma_off] = Shared.gaussianMutationWithSigmaSelfAdaptation(matPool, sigma_pop, algo.lambda, algo.tau_prime, algo.tau, op.bounds);   % mutate
